...
function ticked() {
  ...
  if (isTotalPop) {
    document.getElementById("myBtn").value = "Toggle to Median Income";
    word = "Total Population";
    console.log("enteerd2")
  }

  //if isTotalPop is false, then set the toggle button to be total population and the word in the hover to be Median Income
  if (!(isTotalPop)) {
      document.getElementById("myBtn").value = "Toggle to Total Population";
      word = "Median Income";
...
//   .append('button')
//   .text(toggle ? 'Median Income' : 'Total Population')
//   .on('click', update);
